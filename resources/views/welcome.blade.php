...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <style>
            body {
                ...
                background: linear-gradient(to right, #1a1a1a, crimson);
                color: #fff;
                padding: 50px
            }
            .main-container{
    ...
    <body class="antialiased">
        <div class="relative flex items-top justify-center min-h-screen sm:items-center py-4 sm:pt-0">
            ...
            <div class="main-container max-w-6xl mx-auto sm:px-6 lg:px-8">
                <div class="flex justify-center pt-8 sm:justify-start sm:pt-0">
                    ...
                    <svg class="G-icon" width="24px" height="24px" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                ...
                <div class="mt-8 bg-white dark:bg-gray-800 overflow-hidden shadow sm:rounded-lg">
                    <div class="grid grid-cols-1 md:grid-cols-2">
                        ...
                        <div class="bloco p-6">
                            ...
                            <div class="ml-12">
                                <div class="mt-2 text-gray-600 dark:text-gray-400 text-sm">
                                    @foreach($Google_Console_Search as $key => $value)
                                        <span style="color: #00adff">URL do site: </span> <a style="margin: 8px 0;" href={{ $value['siteUrl'] }} target="_blank">{{ $value['siteUrl'] }}</a><br>
                                    @endforeach
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- bloco 2 -->
                        <div class="bloco p-6 border-t border-gray-200 dark:border-gray-700 md:border-t-0 md:border-l">
                            <div class="flex items-center">
                                <svg width="40px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 461.001 461.001" style="enable-background:new 0 0 461.001 461.001;" xml:space="preserve"><g><path style="fill:#F61C0D;" d="M365.257,67.393H95.744C42.866,67.393,0,110.259,0,163.137v134.728 c0,52.878,42.866,95.744,95.744,95.744h269.513c52.878,0,95.744-42.866,95.744-95.744V163.137 C461.001,110.259,418.135,67.393,365.257,67.393z M300.506,237.056l-126.06,60.123c-3.359,1.602-7.239-0.847-7.239-4.568V168.607 c0-3.774,3.982-6.22,7.348-4.514l126.06,63.881C304.363,229.873,304.298,235.248,300.506,237.056z"/></g></svg>
                                <div class="ml-4 text-lg leading-7 font-semibold text-gray-900 dark:text-white">Dados do Canal {{ $Google_Youtube_Analysis->snippet->title }}</div>
                            </div>

                            <div class="ml-12">
                                <div class="mt-2 text-gray-600 dark:text-gray-400 text-sm">
                                    <div class="flex items-center" style="margin: 15px 0;">
                                        <img src={{ $Google_Youtube_Analysis->snippet->thumbnails->default->url }} alt=""><br>
                                        <div class="ml-4" >
                                            <small style="font-size: 14px;"><span style="color: red; letter-spacing: 2px;">Inscritos: </span> {{ $Google_Youtube_Analysis->statistics->subscriberCount }}</small><br>
                                            <small style="font-size: 14px;"><span style="color: red; letter-spacing: 2px;">Visualizações: </span> {{ $Google_Youtube_Analysis->statistics->viewCount }}</small><br>
                                            <small style="font-size: 14px;"><span style="color: red; letter-spacing: 2px;">Total de videos postados: </span>{{ $Google_Youtube_Analysis->statistics->videoCount }}</small><br>
                                            <small style="font-size: 14px;"><span style="color: red; letter-spacing: 2px;">Publicado: </span>{{ $Google_Youtube_Analysis->snippet->publishedAt }}</small><br>
                                        </div>
                                    </div>

                                    <span class="underline" style="color: red">descrição:</span> {{ $Google_Youtube_Analysis->snippet->description }}<br>
                                </div>
                            </div>
                        </div>

                        <!-- bloco 3 -->
                        <div class="bloco p-6 border-t border-gray-200 dark:border-gray-700">
                            <div class="flex items-center">
                                <svg width="30px" viewBox="-14 0 284 284" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="xMidYMid"><g><path d="M256.003159,247.933017 C256.055907,258.030289 251.77298,267.664804 244.241349,274.390297 C236.709718,281.11579 226.653817,284.285366 216.626905,283.094249 C198.58347,280.424364 185.360959,264.722632 185.800619,246.488035 L185.800619,36.8452103 C185.364944,18.5907614 198.619678,2.88144681 216.687112,0.238996295 C226.704325,-0.933476157 236.743571,2.24455542 244.261279,8.9678962 C251.778988,15.691237 256.053811,25.3147619 256.003159,35.4002282 L256.003159,247.933017 Z" fill="#F9AB00"></path><path d="M35.1010243,213.193238 C54.4867848,213.193238 70.2020487,228.908502 70.2020487,248.294263 C70.2020487,267.680023 54.4867848,283.395287 35.1010243,283.395287 C15.7152639,283.395287 0,267.680023 0,248.294263 C0,228.908502 15.7152639,213.193238 35.1010243,213.193238 Z M127.459466,106.806429 C107.981896,107.874068 92.8698765,124.212107 93.3217628,143.713681 L93.3217628,237.998765 C93.3217628,263.58699 104.580582,279.120548 121.077461,282.431965 C131.434034,284.530959 142.185473,281.860819 150.356699,275.160414 C158.527925,268.460009 163.252393,258.439904 163.222912,247.872809 L163.222912,142.088076 C163.240039,132.641687 159.462041,123.584285 152.737293,116.950107 C146.012546,110.315928 136.904752,106.661084 127.459466,106.806429 L127.459466,106.806429 Z" fill="#E37400"></path></g></svg>
                                <div class="ml-4 text-lg leading-7 font-semibold text-gray-900 dark:text-white">Dados do Google Analytics</div>
                            </div>

                            <div class="ml-12">
                                <div class="mt-2 text-gray-600 dark:text-gray-400 text-sm">
                                    @foreach ($Google_Analytics as $key => $value)
                                        <small><span style="color: yellow;">Url da pagina:</span> {{ $value['url'] }}</small><br>
                                        <small><span style="color: yellow;">Titulo da pagi</span>na: {{ $value['pageTitle'] }}</small><br>
                                        <small><span style="color: yellow;">visualizações:</span> {{ $value['pageViews'] }}</small><br>
                                    @endforeach
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- bloco 4 -->
                        <div class="bloco p-6 border-t border-gray-200 dark:border-gray-700 md:border-l">
                            <div class="flex items-center">
                            <svg width="30px" viewBox="0 -13 256 256" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="xMidYMid"><g><path d="M5.888,166.405103 L90.88,20.9 C101.676138,27.2558621 156.115862,57.3844138 164.908138,63.1135172 L79.9161379,208.627448 C70.6206897,220.906621 -5.888,185.040138 5.888,166.396276 L5.888,166.405103 Z" fill="#FBBC04"></path><path d="M250.084224,166.401789 L165.092224,20.9055131 C153.210293,1.13172 127.619121,-6.05393517 106.600638,5.62496138 C85.582155,17.3038579 79.182155,42.4624786 91.0640861,63.1190303 L176.056086,208.632961 C187.938017,228.397927 213.52919,235.583582 234.547672,223.904686 C254.648086,212.225789 261.966155,186.175582 250.084224,166.419444 L250.084224,166.401789 Z" fill="#4285F4"></path><ellipse fill="#34A853" cx="42.6637241" cy="187.924414" rx="42.6637241" ry="41.6044138"></ellipse></g></svg>
                                <div class="ml-4 text-lg leading-7 font-semibold text-gray-900 dark:text-white">Dados do Google Ads</div>
                            </div>
                ...
                <div class="flex justify-center mt-4 sm:items-center sm:justify-between">
                    <div class="text-center text-sm text-gray-500 sm:text-left">
                        <div class="flex items-center bg-gray-50000">
